def generateUrl(redisCli, taskName, taskNameBuffer, taskNameFp, concurrentCount, queRequest, mulProNums):
...
def redisRun(redisCli, taskName, concurrentCount, queRequest, mulProNums):
    ...
    while redisCli.scard(taskName) > 0:
        generateUrl(redisCli, taskName, taskNameBuffer, taskNameFp, concurrentCount, queRequest, mulProNums)
    # 任务结束的标志
    for i in range(mulProNums):
        queRequest.put(None)


def RedisSave(redisCli, taskName, saveName, concurrentCount, queRequest, mulProNums):
    # 存放url任务的集合key名称
    saveNameBuffer = saveName + "_buffer"
    saveNameFp = saveName + "_fp"

    while True:
        if redisCli.scard(taskName) == 0:
            break
        generateUrl(redisCli, saveName, saveNameBuffer, saveNameFp, concurrentCount, queRequest, mulProNums)
    # 任务结束的标志
    for i in range(mulProNums):
        queRequest.put(None)
